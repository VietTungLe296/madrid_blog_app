...
function UserPosts() {
	...
	const handlePageChange = (pageNumber) => {
		setPage(pageNumber)
		if (keyword !== '') {
			navigate(`/user/posts?page=${pageNumber}&keyword=${keyword.toLowerCase().trim()}`)
		} else {
			navigate(`/user/posts?page=${pageNumber}`)
		}
	}
	...
	useEffect(() => {
		axios
			.get(
				`http://localhost:9000/v1/user/posts?page=${page}&keyword=${keyword.toLowerCase().trim()}`,
	...
	const handleSearchSubmit = (event) => {
		...
		axios
			.get(`http://localhost:9000/v1/user/posts?page=1&keyword=${keyword.toLowerCase().trim()}`, {
				withCredentials: true,
			})
			.then((response) => {
				setPosts(response.data.posts)
				setTotalPosts(response.data.totalPosts)
				setPage(1)
				navigate(`/user/posts?page=1&keyword=${keyword.toLowerCase().trim()}`)
			})
	}

	const handleClearSearch = () => {
		axios
			.get(`http://localhost:9000/v1/user/posts?page=${page}`, { withCredentials: true })
			.then((response) => {
				setPosts(response.data.posts)
				setTotalPosts(response.data.totalPosts)
				setPage(1)
				navigate(`/user/posts?page=1`)
			})
	...
	return (
		<div>
			...
			<div className={styles['home-container']}>
				...
				{totalPosts === 0 ? <EmptyPage /> : <Posts posts={posts} />}
